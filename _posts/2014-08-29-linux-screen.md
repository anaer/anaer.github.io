...
#### linux screen 命令详解
  ...
  C-a d -> detach，暂时离开当前session，将目前的 screen session 丢到后台执行，并会回到还没进 screen 时的状态，此时在 screen session 里    每个 window 内运行的 process 都在继续执行，即使 logout 也不影响。
